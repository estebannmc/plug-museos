...
* Campos de fecha, horarios, ubicación, enlace externo, destacado y estado manual.
...
* Slider de propuestas activas para el inicio del sitio, con filtro opcional por categoría.
* Pantalla administrativa "Ayuda y shortcodes".
...
Muestra propuestas publicadas cuyo estado calculado sea `ACTIVA`. Puede mostrar todas las categorías o filtrar por una o varias.
...
Lista de slugs separados por coma para el slider de propuestas activas, por ejemplo `muestra,taller`.
...
3. Usá el shortcode genérico: `[cc_programacion espacio="slug-del-espacio"]`.

== Agregar nuevos alias de shortcode ==

Para sumar un alias fijo, editá `includes/class-ccp-shortcodes.php` y agregá una entrada al método `aliases()`:

`'nuevoalias' => 'slug-del-espacio',`

Después el alias `[nuevoalias]` va a reutilizar la misma lógica del shortcode genérico.

== Estructura general ==

* `capital-cultural-programacion.php`: bootstrap y cabecera del plugin.
* `includes/`: clases de CPT, taxonomías, metaboxes, shortcodes, estados, fechas, assets y avisos.
* `templates/`: render de grilla, card, modal y mensaje vacío.
* `assets/css/`: estilos frontend y admin.
* `assets/js/frontend.js`: modal sin jQuery.
